...
fun <T, F> Result<T, F>.subscribe(onResult: (T) -> Any, onFailure: (F) -> Any): Any =
    when (this) {
        is Result.Success -> onResult(value)
        is Result.Failure -> onFailure(failure)
    }

fun <T, F> Result<T, F>.subscribe(onResult: (T?, F?) -> Any): Any =
    when (this) {
        is Result.Success -> onResult(value, null)
        is Result.Failure -> onResult(null, failure)
    }

fun <F> Result<Unit, F>.complete(onResult: () -> Any, onFailure: (F) -> Any): Any =
    when (this) {
...
inline fun <reified V, reified F, reified R> Result<V, F>
